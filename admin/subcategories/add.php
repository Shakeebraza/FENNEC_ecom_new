...
$datacat= $dbFunctions->getData('categories');
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $lastid = $dbFunctions->getLastIdshort('subcategories');
    ...
    if ($category_id) {
        ...
        $updateResult = $dbFunctions->setData('subcategories', $addNewData);

        if ($updateResult['success']) {
            echo "<script>alert('Subcategory added successfully.');</script>";
        } else {
            echo "<script>alert('Error adding subcategory: {$updateResult['message']}');</script>";
        }
    } else {
                            ...
                            <div class="form-group">
                                ...
                                <select id="category" name="category_id" class="form-control" required>
                                    ...
                                    foreach($datacat as $val){
                                        echo '<option value="'.$security->decrypt($val['id']).'">'.$security->decrypt($val['category_name']).'</option>';
                                    }
                                    ?>
                                </select>
                            </div>
