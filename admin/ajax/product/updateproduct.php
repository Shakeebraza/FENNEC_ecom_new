...
try {
    ...
    $productData = [
        ...
        'price' => $_POST['price'],
        'product_type' => $_POST['membershipType']
    ];
    ...
    if (!empty($_POST['deleteGalleryImages'])) {
        $deleteImages = json_decode($_POST['deleteGalleryImages'], true);
        $deleteStmt = $pdo->prepare("DELETE FROM product_images WHERE product_id = :product_id AND image_path = :image_path");
        foreach ($deleteImages as $imagePath) {
            $deleteStmt->execute([':product_id' => $productId, ':image_path' => $imagePath]);
            @unlink('../../../' . $imagePath);
        }
    }
